...
- Intake validator modes for release policy correctness: baseline repository validation (`npm run intake:validate` / `--mode=repo`) vs strict local forensic validation (`npm run intake:validate:forensic` / `--mode=forensic`) so CI does not require local heavy forensic archives at repo root while provenance fingerprints remain enforced.
